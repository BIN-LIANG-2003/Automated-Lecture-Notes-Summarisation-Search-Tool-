...
- `EXTERNAL_OCR_SERVICE_URL`, `EXTERNAL_OCR_AUTH_TOKEN`, `EXTERNAL_OCR_TIMEOUT_SECONDS`: optional external OCR service. The Modal OCR service itself should also set `OCR_SERVICE_AUTH_TOKEN`.
...
- Query-string auth is no longer generated by the React preview UI. It remains accepted only as transitional compatibility for older `/api/documents/:id/file?auth_token=...` links.
...
- The React app stores only a cookie-session marker in browser storage; the real session token is carried by an HTTP-only same-origin cookie.
- Default Flask responses include security headers such as CSP, `X-Content-Type-Options`, `X-Frame-Options`, and `Permissions-Policy`.
