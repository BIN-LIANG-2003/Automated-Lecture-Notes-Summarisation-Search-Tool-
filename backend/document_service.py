...
from .config import CANVAS_DEFAULT_DOMAIN, CANVAS_MAX_LIST_PAGES, DEFAULT_DOCUMENT_CATEGORY, DEFAULT_WORKSPACE_SETTINGS, MIME_BY_EXT, S3_BUCKET, s3_client, TRASH_RETENTION_DAYS
...
def get_canvas_files():
    ...
    try:
        ...
        supported_files.sort(key=lambda item: str(item.get('updated_at') or ''), reverse=True)
        return jsonify({
    ...
    try:
        ...
        for chunk in file_res.iter_content(chunk_size=262144):
            ...
            if max_size and total_bytes > max_size:
                return jsonify({'error': f'Downloaded file exceeded size limit ({max_size} bytes).'}), 400
            chunks.append(chunk)
        ...
        try:
            ...
            doc_payload = row_to_dict(cursor.fetchone()) or {}
            imported_ok = True
            return jsonify({
